...
export default function UpdateQuestions({ initialData, onBack }: CreateSectionsProps) {
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        try {
            ...
            const response = await patchData(`${apiUrls.postQuestions}/${initialData?.data?._id}`, formData, true)
            if (response) {
                onBack()
            }
        } catch (error: any) {
                    ...
                    <div className="pl-9 mt-6">
                        ...
                        <form className='flex flex-col gap-5 text-lg text-black-400 pb-[92px]'>
                            ...
                            <div className='flex flex-col gap-1 w-full'>
                                <FileUpload allowedFileTypes={[".png",".jpg", ".jpeg"]} fileUrl={formData.thumbnail} label={"Thumbnail"} text={"Add Image"} onFileSelected={(file) => {
